...
async function geocodeWithGoogle(
	...
	if (!apiKey) {
		console.log(
			"[RouteMapImage] Google Maps API key not set, trying Nominatim...",
		);
		return null;
	}
	...
	try {
		...
		if (data.status === "OK" && data.results?.[0]?.geometry?.location) {
			...
			console.log(
				`[RouteMapImage] Google geocoded ${address} to (${lat}, ${lng})`,
			);
			return { lat, lng };
		}

		console.log(
			`[RouteMapImage] Google geocoding failed: ${data.status}, trying Nominatim...`,
		);
		return null;
	} catch (error) {
...
async function geocodeWithNominatim(
	...
	try {
		...
		if (data && data.length > 0 && data[0].lat && data[0].lon) {
			...
			console.log(
				`[RouteMapImage] Nominatim geocoded ${address} to (${lat}, ${lng})`,
			);
			return { lat, lng };
		}
...
function selectBestStation(
	...
	if (maisokuStationName) {
		...
		const matchedStation = apiStations.find(
			(s) =>
				s.name === maisokuStationName ||
				s.name.includes(maisokuStationName) ||
				maisokuStationName.includes(s.name),
		);
...
export const generateRouteMapImageTool = tool({
	...
	execute: async (params: RouteMapImageInput): Promise<RouteMapImageResult> => {
		...
		let nearestStationInfo = selectBestStation(
			apiStations,
			nearestStationName,
			walkMinutes,
		);
		...
		if (!nearestStationInfo) {
			if (nearestStationName) {
				...
				console.log(
					`[RouteMapImage] Using maisoku station only: ${nearestStationName}`,
				);
			} else {
		...
		const airportsData = routeMapData.airportRoutes.map((route) => ({
			name: route.destination,
			lat: AIRPORTS[route.destination as keyof typeof AIRPORTS]?.lat || 35.5494,
			lng:
		...
		try {
			...
			const blob = new Blob([Buffer.from(screenshotBuffer)], {
				type: "image/png",
			});
