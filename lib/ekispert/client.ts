...
export class EkispertClient {
		...
		for (const [area, keywords] of Object.entries(AREA_KEYWORDS)) {
			for (const keyword of keywords) {
				if (address.includes(keyword)) {
					console.log(
						`[Ekispert] Detected area: ${area} (matched: ${keyword})`,
					);
	...
	private getStationsForArea(
		...
		console.log(
			`[Ekispert] Searching ${stations.length} stations for area: ${area} (${searchAreas.join(", ")})`,
		);
		return stations;
	}
	...
	async searchRoute(
		...
		try {
			...
			console.log(
				`[Ekispert] Searching route: ${fromStationCode} -> ${toStationCode}`,
			);
	...
	private async fetchRouteData(
		...
		const stationRoutePromises = targetStations.map(async (station) => {
			const route = await this.searchRoute(nearestStation.code, station.code);
			if (route) {
				return {
					...route,
		...
		const airportRoutePromises = AIRPORTS.map(async (airport) => {
			const route = await this.searchRoute(nearestStation.code, airport.code);
			if (route) {
				return {
					...route,
