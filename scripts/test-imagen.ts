...
async function testPropertyImage(client: RealEstateImagenClient) {
  ...
  if (!SAMPLE_MAISOKU_URL) {
    console.log("TEST_MAISOKU_URL 環境変数が設定されていません。スキップします。")
    return
  }
  ...
  if (result.image) {
    ...
    if (result.intermediateImage) {
      ...
      console.log(`  - サイズ: ${result.intermediateImage.imageData.length} bytes`)
    }
...
async function main() {
  ...
  switch (testType) {
    ...
    default:
      ...
      console.log("使用可能: property, floorplan, routemap, routemap-address, all")
      process.exit(1)
  }
